...
- Keep SpawnerProtect `/sell` cleanup split into two shifted passes across
  ...
  on the server.
- SpawnerProtect `/sell` cleanup should only run when player inventory has
  sellable stacks. Ground item entities should not force `/sell` loops; once
  `min-empty-slots-before-break` is satisfied, continue to breaking immediately.
