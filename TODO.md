...
- Description: Limit SpawnerProtect `/sell` cleanup to inventory items.
  Priority: Critical
  Status: Completed
  Files: `src/main/java/com/kami/order/modules/KamiSpawnerProtect.java`,
    `README.md`, `AGENTS.md`, `TODO.md`, `gradle.properties`
  Notes: `isBreakPickupSafeNow()` now depends on the configured empty-slot
    requirement only. `/sell` cleanup starts only when the player inventory has
    sellable stacks, and it stops immediately once enough empty slots are
